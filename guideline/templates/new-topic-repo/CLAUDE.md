...
10. **Show the user what happened** — the commit message and the output of
    ...
    without checking themselves. Then ask one more light, optional
    question, the same "ask, don't run" shape as step 8: **"Want me to run
    `/update_paper` now to check for wrong facts, contradictions, or
    misplaced citations?"** Run it only if they say yes; if they decline or
    don't answer, move on — never run it automatically (see the manual's
    Section 18 note on why this isn't wired into the after-every-commit
    hook: it's a full reasoning pass, not a fast deterministic check, and
    running it uninvited on every commit is exactly the kind of delay that
    gets a check disabled). Asking costs nothing; running it is the user's
    call every time.
